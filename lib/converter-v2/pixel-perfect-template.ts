...
function estimateInitialHeight($: cheerio.CheerioAPI) {
  ...
  return Math.max(1200, sectionCount * 420 + imageCount * 120);
}
...
function createIframeOnloadScript() {
  ...
  function bindObservers() {
    if (frame.__htmlToElementorBound) return;
    var doc = getFrameDocument();
    if (!doc) return;
    frame.__htmlToElementorBound = true;
    try {
    ...
    try {
      ...
      if (Observer) {
        ...
        frame.__htmlToElementorResizeObserver = resizeObserver;
      }
    } catch (error) {}
  }
  bindObservers();
...
function injectFrameRuntime(html: string, frameToken: string) {
  const $ = cheerio.load(
    html.replace(/\b100(?:d|s)?vh\b/g, "var(--html-to-elementor-fixed-vh, 100vh)")
  );
  ...
  $("head").append(`<style data-html-to-elementor-frame-runtime>
:root {
  --html-to-elementor-fixed-vh: 100vh;
}
...
(function() {
  ...
  function notifyParent() {
    ...
    parent.postMessage(
      {
        type: "html-to-elementor:frame-resize",
        token: frameToken,
        height: height
      },
      "*"
...
export function createPixelPerfectElementorDocument(
  html: string,
  title = "Elementor Page"
): ElementorDocument {
  const $ = cheerio.load(html);
  const documentTitle = $("title").first().text().trim() || title;
  ...
  class="html-to-elementor-frame"
  data-html-to-elementor-frame="${frameToken}"
...
(function(){
  ...
  function handleFrameMessage(event) {
    ...
    if (!data || data.type !== "html-to-elementor:frame-resize" || data.token !== frameToken) return;
    ...
    content: [
      {
        ...
        settings: {
          ...
          html_to_elementor_strategy: "pixel-perfect-iframe-v2"
        },
        elements: [
          {
            ...
            elements: [
              {
                ...
                settings: {
                  html: widgetHtml,
                  lovable_clone_mode: "iframe_srcdoc_pixel_perfect_v2",
                  lovable_clone_note:
                    "Site original encapsulado em iframe srcdoc para impedir interferencia visual do Elementor ou do tema."
                },
                elements: []
              }
            ]
